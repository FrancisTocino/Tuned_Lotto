...
function importacion_loterias( $fileName ){
		...
		$fName=$fileName;
		$fp=fopen($fName,"r") or die('<li>Fallo al abrir el fichero: '.$fileName.'...'. mysqli_error(). '</li><br>');
		...
		while(!feof($fp))
		{
			...
			echo '</br> $fecha_bonoloto = ' . $fecha_bonoloto;
			switch ($fecha_bonoloto)
			{
				case (strpos($fecha_bonoloto, 'FECHA') !== false):
...
//echo('<br />$file_name = '.$file_name = $_FILES['NombreFichero']['name']);   
   ...
   // echo('<br />$targetFile = '.$targetFile =  str_replace('//','/',$targetPath) . $file_name);
	...
	echo '</br>$tempFile: '  .$targetPath; 	
	echo '</br>$fileName: '  .$targetFile;
